...
from sql.sql_connect_vars import (
    KEY_VALUE_NAME_DATABASE,
    KEY_VALUE_NAME_USER,
    KEY_VALUE_NAME_HOST,
    KEY_VALUE_NAME_PASS,
    KEY_VALUE_NAME_PORT)
...
class csql_eng:
    ...
    @staticmethod
    def get_value_name_database():
        return KEY_VALUE_NAME_DATABASE

    @staticmethod
    def get_value_name_user():
        return KEY_VALUE_NAME_USER

    @staticmethod
    def get_value_name_host():
        return KEY_VALUE_NAME_HOST

    @staticmethod
    def get_value_name_port():
        return KEY_VALUE_NAME_PORT

    @staticmethod
    def get_value_name_pass():
        return KEY_VALUE_NAME_PASS
    ...
    def is_valid_saved_connect_data(self) -> bool:  # Проверит сохранённую инфу о подключении
        ...
        if self.check_connect_data(cenum.SQL_CONNECT_DATA_TYPE.DB_NAME,
                                   str(self.__db_connect_data[KEY_VALUE_NAME_DATABASE])) is None:
            return False
        if self.check_connect_data(cenum.SQL_CONNECT_DATA_TYPE.DB_NAME,
                                   str(self.__db_connect_data[KEY_VALUE_NAME_USER])) is None:
            return False
        if self.check_connect_data(cenum.SQL_CONNECT_DATA_TYPE.DB_NAME,
                                   str(self.__db_connect_data[KEY_VALUE_NAME_HOST])) is None:
            return False
        if self.check_connect_data(cenum.SQL_CONNECT_DATA_TYPE.DB_NAME,
                                   str(self.__db_connect_data[KEY_VALUE_NAME_PORT])) is None:
            return False
        if self.check_connect_data(cenum.SQL_CONNECT_DATA_TYPE.DB_NAME,
                                   str(self.__db_connect_data[KEY_VALUE_NAME_PASS])) is None:
            return False
    ...
    def sql_connect(self) -> bool | object:
        ...
        try:
            ...
            connect_handle = psycopg2.connect(host=self.__db_connect_data[KEY_VALUE_NAME_HOST],
                                              ...
                                              password=self.__db_connect_data[KEY_VALUE_NAME_PASS],
                                              port=self.__db_connect_data[KEY_VALUE_NAME_PORT])

            cur = connect_handle.cursor()
            # if current_time_zone == TIME_ZONE.RF:
